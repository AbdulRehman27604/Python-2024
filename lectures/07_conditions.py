...
'''
def print_test(test: str, is_true: bool):
    """
    >>> print_test("Science", True)
    Science Passed
    >>> print_test("Math", False)
    Math Failed
    """
    if is_true:
        print(test,":","Passed")
...
with a maximum of 9 teammates on a team.
'''
def num_teams(split_into_teams , max_team_size):
    """
    >>> num_teams(9,9)
    teams: 1
    >>> num_teams(21,9)
    ...
    teams: 3
    """
    teams = (split_into_teams // max_team_size) 
    if split_into_teams % max_team_size > 0:
        teams = teams + 1
        print("teams",":",teams)
    else:
        print("teams",":",teams
