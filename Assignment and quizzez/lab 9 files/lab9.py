...
def most_freq_number(filename: str) -> list[int]:
    ...
    dictionary_of_number = get_number_frequencies(filename)
    most_frequency = 0
    result_list = []
    
    for number in dictionary_of_number:
        frequency = dictionary_of_number[number]
...
37: ['Liechtenstein']})
    ...
    for line in file_handle:
        line = line.strip()
        seperated_list = line.split(',')
        country_name = seperated_list[COUNTRY]
        population = int(seperated_list[POP])
        country_population[country_name] = population
        granularity_pop = population // granularity
        
        if granularity_pop not in population_bin:
            population_bin[granularity_pop] = [country_name]
        else:
            population_bin[granularity_pop].append(country_name)
            
    file_handle.close()
    result = (country_population, population_bin)
    return result
...
(2876101, 'Albania'), (2881355, 'Jamaica'), (2924816, 'Armenia')]
    ...
    country_bin = 0
    result_list = []
    population_dict = population_tuple[TUPLE_1]
    granularity_dict = population_tuple[TUPLE_2]
    
    if country not in population_dict:
        result_list =  []
    else:
        country_bin = population_dict[country] // granularity
    
        lst = granularity_dict[country_bin]
        for location in lst:
            population = population_dict[location]
            information = population, location
            result_list.append(information)
        result_list.sort()
